...
export function generateSingleSCOHTML(
  ...
  const moduleSlidesHTML = modules.map((module, index) => {
    ...
    const sectionsHTML = module.content.map((section) => {
      ...
      // Videos get special styling
      if (section.type === 'video') {
        const videoSection = section as any;
        const isEmbed = videoSection.videoType === 'embed';
        return `
      <div class="content-card card-video" style="border: none; box-shadow: none; background: transparent;">
        ${section.heading ? `<div class="card-header" style="background: linear-gradient(135deg, #fff7ed 0%, #ffedd5 100%); border-radius: 8px 8px 0 0;"><span class="title-icon" style="color: #ea580c;">🎬</span><h3 style="color: #9a3412;">${escapeHtml(section.heading)}</h3></div>` : ''}
        <div class="card-content" style="text-align: center; padding: 0;">
          ${isEmbed ? `
          <iframe 
            src="${videoSection.embedUrl || section.body}" 
            style="width: 100%; aspect-ratio: 16/9; max-height: 400px; border-radius: 8px; border: none; display: block;"
          ...
          <video 
            src="${section.body}" 
            controls 
            style="max-width: 100%; max-height: 400px; border-radius: 8px; display: block; margin: 0 auto;"
          >
      ...
      // Important/Warning callouts - no checkbox, clean styling
      if (isInteractive) {
        return `
