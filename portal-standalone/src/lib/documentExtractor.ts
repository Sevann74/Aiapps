...
// ============================================
// DETERMINISTIC DOCUMENT COMPARISON ENGINE
// Based on ComplianceQueryPro approach
// Simple section-level diffing, no inference
// ============================================
...
export function compareDocuments(oldDoc: ExtractedDocument, newDoc: ExtractedDocument): ComparisonResult {
  ...
  for (const [id, oldSection] of oldSectionMap) {
    ...
    if (newSection) {
      // Normalize for comparison (whitespace, case)
      const oldNorm = normalizeText(oldSection.content);
      const newNorm = normalizeText(newSection.content);
      ...
      // Section removed in new document
      changes.push({
        sectionId: id,
  ...
  // Find new sections (exist in new but not in old)
  ...
  const totalSections = Math.max(oldSectionMap.size, newSectionMap.size);
  ...
  return {
    ...
    summary: {
      ...
      modifiedSections: changes.filter(c => c.changeType === 'modified').length,
      newSections: changes.filter(c => c.changeType === 'added').length,
      removedSections: changes.filter(c => c.changeType === 'removed').length
    }
  };
}
