...
import { signIn, signOut, createUser, deleteUser, setUserActive, getAllClients, updateUserProfile, requestPasswordReset, getCurrentUser, type UserProfile } from './lib/authService';
import { supabase } from './lib/supabaseClient';
import { HubHome, HubAdmin, SOPComparisonTool, CourseBuilder, DocumentQA } from './components/Hub';
...
const StreamlinedCourseBuilder = () => {
  ...
  const [clientForm, setClientForm] = useState({
    ...
    unlimitedAttempts: false, // Allow unlimited quiz attempts
    ...
    comments: '',
    includePdfDownload: false // Allow learners to download PDF version
  });
  
  // Image uploads with placement info
  const [uploadedImages, setUploadedImages] = useState<Array<{id: string; file: File; data: string; placement: string}>>([]);
  const imageInputRef = useRef<HTMLInputElement>(null);
  
  const [uploadedFile, setUploadedFile] = useState(null);
  const [fileChecksum, setFileChecksum] = useState('');
  const [previousVersionFile, setPreviousVersionFile] = useState(null);
  const [previousFileChecksum, setPreviousFileChecksum] = useState('');
  const [confirmCheckbox, setConfirmCheckbox] = useState(false);
  const [manualQuestions, setManualQuestions] = useState<any[]>([]);
  const [showQuestionEditor, setShowQuestionEditor] = useState(false);
  const [editingQuestion, setEditingQuestion] = useState<any>(null);
  const [companyLogo, setCompanyLogo] = useState<{name: string; data: string; colors?: {primary: string; secondary: string; dark: string; light: string}} | null>(null);
  ...
  // Validate/restore Supabase session on mount - handle stale tokens AND restore valid sessions
  useEffect(() => {
    const validateAndRestoreSession = async () => {
      try {
        ...
        // Session valid - verify user profile and restore React state
        if (data.session.user) {
          ...
          if (!profile) {
            ...
            // RESTORE React state from valid Supabase session
            console.log('Valid session found, restoring React state');
            setCurrentUser(profile);
            setIsAuthenticated(true);
            setCurrentView('hub');
            localStorage.setItem('coursebuilder_session', JSON.stringify({ isAuthenticated: true, user: profile }));
          }
        }
      } catch (err) {
        // Auth error (e.g., invalid refresh token) - clear and show login
    ...
    // Always validate on mount - restore session if Supabase has one
  ...
  const handlePreviousFileUpload = async (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    reader.onload = async (event) => {
      try {
        ...
        const hashArray = Array.from(new Uint8Array(hashBuffer));
        const checksum = hashArray.map(b => b.toString(16).padStart(2, '0')).join('');
        setPreviousFileChecksum(checksum);
      } catch (err) {
        // Fallback for older browsers
        const checksum = btoa(String(event.target.result).substring(0, 64));
        setPreviousFileChecksum(checksum);
  ...
  const handleSubmitJob = async () => {
    ...
    const effectiveEmail = clientForm.contactEmail || currentUser?.email;
    if (!effectiveEmail) {
    ...
    // Upload current document to Supabase Storage
    ...
    // Upload previous version document if provided (for version comparison)
    let previousUploadResult = null;
    if (previousVersionFile) {
      previousUploadResult = await uploadDocument(previousVersionFile, `${jobId}-previous`, currentUser.id);
      if (!previousUploadResult.success) {
        alert(`Failed to upload previous version document: ${previousUploadResult.error}`);
        return;
      }
    }
    ...
    const newJob = {
      ...
      previousFileName: previousVersionFile?.name || null,
      previousFilePath: previousUploadResult?.path || null,
      previousFileChecksum: previousFileChecksum || null,
      hasPreviousVersion: !!previousVersionFile,
      ...
      auditLog: [
        {
          ...
          details: `File: ${uploadedFile.name}, Checksum (SHA-256): ${fileChecksum}${previousVersionFile ? `, Previous version: ${previousVersionFile.name}` : ''}`,
          ip: 'client-side'
        },
        {
          timestamp: new Date().toISOString(),
          ...
          actor: 'system',
          details: `Path: ${uploadResult.path}${previousUploadResult ? `, Previous: ${previousUploadResult.path}` : ''}`,
          ip: 'server'
        }
      ]
    };
...
Document Version Number:          ${job.sopNumber || 'N/A'}
...
════════════════════════════════════════════════════════════════════════════════`;
  ...
  const downloadAuditTrail = (job, isClientView = false) => {
    ...
    w.document.write('<div class="fld"><span class="fld-l">Document Version Number:</span><span>' + (job.sopNumber || 'N/A') + '</span></div>');
  ...
  const clientApprovePreview = async (job) => {
    const now = new Date().toISOString();
    
    // Update job in Supabase (approvedBy must be UUID, not email)
    const jobUpdates = {
      status: 'delivered',
      approvedAt: now,
    ...
    const result = await updateJob(job.id, jobUpdates, currentUser.id, 'Job approved by client');
    if (!result.success) {
      console.error('Failed to update job in database:', result.error);
      alert('❌ Approval Failed\n\nCould not save approval. Please try again.');
      return;
    }
    ...
    // Update selectedJob to reflect new status so UI updates immediately
    const updatedJob = updatedJobs.find(j => j.id === job.id);
    if (updatedJob) {
      setSelectedJob(updatedJob);
    }
    
    showNotification('success', 'Job Approved!', 'Your SCORM package and Audit Trail are now available for download below.');
  };
  
  const clientRequestRevision = async (job, comment) => {
    ...
    const result = await updateJob(job.id, jobUpdates, currentUser.id, 'Revision requested');
    if (!result.success) {
      console.error('Failed to update job in database:', result.error);
      alert('❌ Request Failed\n\nCould not save revision request. Please try again.');
      return;
    }
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-gray-900 mb-2">CapNorth Hub</h1>
            <p className="text-gray-600">Enterprise Learning & Compliance Platform</p>
          </div>
  ...
  // ============================================
  // HUB ADMIN - Organizations & Entitlements Management
  // ============================================

  if (currentView === 'hub-admin' && currentUser.role === 'admin') {
    return (
      <HubAdmin onBack={() => setCurrentView('hub')} />
    );
  }
  ...
  // ============================================
  // DOCUMENT Q&A / INSIGHT SEARCH
  // ============================================

  if (currentView === 'doc-qa') {
    return (
      <DocumentQA
        onBack={() => setCurrentView('hub')}
        user={{
  ...
  if (currentView === 'course-builder') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-50">
        {/* Header */}
        <div className="bg-gradient-to-r from-purple-900 via-indigo-900 to-purple-900 text-white py-4 px-6 shadow-lg">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold tracking-tight">Learning Conversion Workspace</h1>
                <p className="text-xs text-purple-200">Direct L&D Access to Learning Conversion</p>
              </div>
            </div>
            <button
        ...
        <ErrorBoundary fallbackMessage="The Course Builder encountered an error. Your work may be auto-saved. Please try again.">
          <CourseBuilder />
        </ErrorBoundary>
      </div>
    );
  ...
  // ============================================
  // HUB HOME - Main landing page after login
  // ============================================

  
  if (currentView === 'hub') {
    const activeJobsCount = jobs.filter(j => 
      ['submitted', 'in_progress', 'pending_review', 'revision_requested'].includes(j.status)
    ).length;
    ...
    const handleSelectModule = (moduleKey: string) => {
      ...
      } else if (moduleKey === 'course-builder') {
        setCurrentView('course-builder');
      } else if (moduleKey === 'sop-compare') {
        setCurrentView('sop-compare');
      } else if (moduleKey === 'doc-qa') {
        setCurrentView('doc-qa');
      } else if (moduleKey === 'compliance') {
        alert('Compliance Query Pro coming soon!');
      } else if (moduleKey === 'hub-admin') {
        setCurrentView('hub-admin');
      }
    };
        ...
        <div className="bg-gradient-to-r from-indigo-900 via-purple-900 to-indigo-900 text-white py-4 px-6 shadow-lg mb-6">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold tracking-tight">Learning Conversion</h1>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
            <div className="flex gap-4">
              <input
                type="text"
                placeholder="Search by course ID, title, or Document Version Number..."
                value={searchQuery}
            ...
            <div className="space-y-6">
              ...
              <div className="p-4 bg-blue-50 border-2 border-blue-200 rounded-lg">
                ...
                <ul className="text-sm text-blue-800 space-y-1">
                  <li>• <strong>Format:</strong> <span className="text-green-700 font-bold">Word (.docx) is highly preferred</span> - works better than PDF</li>
                  <li>• <strong>Maximum:</strong> 30 pages or 75,000 characters</li>
                  <li>• <strong>Images:</strong> <span className="text-purple-700 font-semibold">Image extraction only works with Word documents</span> - use the image upload below for PDFs</li>
                </ul>
                <div className="mt-3 pt-3 border-t border-blue-200">
                  <p className="text-xs font-semibold text-blue-900 mb-1">⚠️ Not Supported:</p>
                  <p className="text-xs text-blue-700">Scanned/image-only PDFs • Password-protected files • Fillable forms • Digital signatures • Annotations/comments</p>
                </div>
              </div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-blue-500 transition-all cursor-pointer"
                  ...
                  {uploadedFile ? (
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">Word (.docx) preferred • PDF accepted • Max 50MB • Up to 30 pages</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 mb-3">
                  Add images to include in your course. Specify where each image should be placed.
                  <span className="text-purple-600 font-semibold"> Required for PDF documents (image extraction only works with Word).</span>
                </p>
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-green-500 transition-all cursor-pointer"
                  ...
                  <input
                    ...
                    className="hidden"
                  />
                  <Icon name="upload" className="w-10 h-10 mx-auto mb-2 text-gray-400" style={{fontSize: '40px'}} />
                  <p className="text-gray-600 font-semibold">Click to upload images</p>
                  <p className="text-sm text-gray-500">PNG, JPG, GIF • Multiple files allowed</p>
                </div>
                ...
                {uploadedImages.length > 0 && (
                  <div className="mt-4 space-y-3">
                    {uploadedImages.map((img, idx) => (
                      <div key={img.id} className="flex items-start gap-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
                        <img src={img.data} alt={`Upload ${idx + 1}`} className="w-16 h-16 object-cover rounded" />
                        <div className="flex-1">
                          <p className="font-semibold text-gray-900 text-sm">{img.file.name}</p>
                          <input
                            type="text"
                            value={img.placement}
                            onChange={(e) => {
                              setUploadedImages(prev => prev.map(i => 
                                i.id === img.id ? {...i, placement: e.target.value} : i
                              ));
                            }}
                            placeholder="Where should this image go? e.g., 'After Introduction section' or 'Module 2, before Safety Steps'"
                            className="w-full mt-2 px-3 py-2 text-sm border border-gray-300 rounded focus:border-blue-500 focus:outline-none"
                          />
                        </div>
                        <button
              ...
              <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
                <input
                  type="checkbox"
                  id="pdfDownload"
                  checked={clientForm.includePdfDownload}
                  onChange={(e) => setClientForm({...clientForm, includePdfDownload: e.target.checked})}
                  className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500"
                />
                <label htmlFor="pdfDownload" className="flex-1">
                  <span className="font-semibold text-gray-900">Make PDF version available for download</span>
                  <p className="text-sm text-gray-600">Learners will be able to download a PDF copy of the training document within the course</p>
                </label>
              </div>
              
              {/* Previous Version Upload - OPTIONAL */}
              <div>
                <label className="block text-sm font-bold text-gray-900 mb-2">
                  Previous Document Version <span className="text-gray-500 font-normal">(optional)</span>
                </label>
                <p className="text-sm text-gray-600 mb-3">
                  If a previous version is provided, a "What Changed" section will be included at the start of the course.
                  <span className="text-orange-600 font-semibold"> File format must match the current version for best comparison results.</span>
                </p>
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-purple-500 transition-all cursor-pointer"
                     onClick={() => previousFileInputRef.current?.click()}>
                  <input
                    ref={previousFileInputRef}
                    type="file"
                    accept=".pdf,.docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                    onChange={handlePreviousFileUpload}
                    className="hidden"
                  />
                  {previousVersionFile ? (
                    <div className="flex items-center justify-center gap-4">
                      <Icon name="file" className="w-10 h-10 text-purple-600" style={{fontSize: '40px'}} />
                      <div className="text-left">
                        <p className="font-bold text-gray-900">{previousVersionFile.name}</p>
                        <p className="text-sm text-gray-600">Previous version for comparison</p>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setPreviousVersionFile(null);
                    ...
                    <div>
                      <Icon name="upload" className="w-10 h-10 mx-auto mb-2 text-gray-400" style={{fontSize: '40px'}} />
                      <p className="text-gray-600 font-semibold">Click to upload previous version</p>
                      <p className="text-sm text-gray-500">Word (.docx) preferred • PDF accepted</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  type="email"
                  value={clientForm.contactEmail || currentUser?.email || ''}
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        disabled={clientForm.unlimitedAttempts}
                        className={`w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:outline-none font-semibold ${clientForm.unlimitedAttempts ? 'bg-gray-100 text-gray-400' : ''}`}
                      />
                      <div className="flex items-center gap-2 mt-2">
                        <input
                          type="checkbox"
                          id="unlimitedAttempts"
                          checked={clientForm.unlimitedAttempts}
                          onChange={(e) => setClientForm({...clientForm, unlimitedAttempts: e.target.checked})}
                          className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500"
                        />
                        <label htmlFor="unlimitedAttempts" className="text-sm text-gray-700">Allow unlimited attempts</label>
                      </div>
                    </div>
                  </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg p-6 border-2 border-gray-100">
                ...
                {/* Simplified Logo Guidelines */}
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-700">
                  <p className="mb-1"><strong>Logo tips:</strong> PNG with transparent background • Max 2MB • Horizontal logos work best</p>
                  <p className="text-blue-600"><strong>💡 Note:</strong> You only need to upload your logo once. It will be saved to your account and automatically applied to future courses. Re-upload only if your logo changes.</p>
                </div>
                      ...
                      if (file) {
                        ...
                        reader.onload = (event) => {
                          ...
                          img.onload = () => {
                            ...
                            if (imageData2) {
                              // Simple color extraction - find most common non-white/black colors
                              ...
                              for (let i = 0; i < imageData2.data.length; i += 4) {
                                ...
                                // Skip transparent, white-ish, and black-ish pixels
                                if (a < 128) continue;
                              ...
                              if (sorted.length > 0) {
                                ...
                                const secondary = sorted.length > 1 ? `rgb(${sorted[1][0].split(',').join(', ')})` : primary;
                                ...
                                setCompanyLogo({ name: file.name, data: imageData });
                              }
                            } else {
                              setCompanyLogo({ name: file.name, data: imageData });
                            }
                  ...
                  {companyLogo ? (
                    <div className="p-4 bg-green-50 border-2 border-green-200 rounded-lg">
                      <div className="flex items-center gap-4 mb-3">
                        <img src={companyLogo.data} alt="Logo preview" className="w-16 h-16 object-contain rounded bg-white p-1" />
                        <div className="flex-1">
                      ...
                      {companyLogo.colors && (
                        <div className="pt-3 border-t border-green-200">
                          <p className="text-xs font-semibold text-gray-700 mb-2">🎨 Extracted Brand Colors:</p>
                          <div className="flex items-center gap-2">
                            <div className="w-6 h-6 rounded border-2 border-white shadow" style={{ backgroundColor: companyLogo.colors.primary }} title="Primary" />
                            <div className="w-6 h-6 rounded border-2 border-white shadow" style={{ backgroundColor: companyLogo.colors.secondary }} title="Secondary" />
                            <div className="w-6 h-6 rounded border-2 border-white shadow" style={{ backgroundColor: companyLogo.colors.dark }} title="Dark" />
                            <div className="w-6 h-6 rounded border-2 border-white shadow" style={{ backgroundColor: companyLogo.colors.light }} title="Light" />
                            <span className="text-xs text-green-700 ml-2">✓ Colors will be applied to course theme</span>
                          </div>
                        </div>
                      )}
                    </div>
              ...
              <div key={job.id} className="bg-white rounded-2xl shadow-xl p-6 hover:shadow-2xl transition-all">
                ...
                <div className="grid grid-cols-5 gap-4 mb-4">
                  <div className="bg-purple-50 rounded-lg p-3">
                    <div className="text-xs text-purple-600 mb-1">Document Version Number</div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
            ...
            <div className="p-4 bg-purple-50 border-2 border-purple-200 rounded-lg">
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <div className="text-xs text-purple-600 mb-1">Name</div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                <label className="text-sm font-semibold text-gray-600">Document Version Number</label><p className="text-lg text-gray-900">{selectedJob.documentVersion || 'Not provided'}</p>
              </div>
              <div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
            ...
            {/* Previous Version Document - if provided */}
            {selectedJob.hasPreviousVersion && selectedJob.previousFileName && (
              <div className="flex items-center gap-4 p-4 bg-purple-50 border-2 border-purple-200 rounded-lg mb-4">
                <Icon name="file" className="w-12 h-12 text-purple-600" style={{fontSize: '48px'}} />
                <div className="flex-1">
                  <p className="font-semibold text-gray-900">{selectedJob.previousFileName}</p>
                  <p className="text-sm text-purple-600 font-medium">Previous Version (for comparison)</p>
                  {selectedJob.previousFileChecksum && (
                    <p className="text-sm text-gray-600">Checksum: {selectedJob.previousFileChecksum}</p>
                  )}
                </div>
                <button
                  onClick={() => {
                      ...
                      if (j.id === job.id) {
                        return {
                          ...
                          auditLog: [
                            ...
                            {
                              timestamp: new Date().toISOString(),
                              action: 'Admin downloaded previous version SOP',
                              actor: currentUser.email,
                              details: `File: ${job.previousFileName}`
                            }
                          ]
                        };
                      }
                    ...
                    const fileContent = `Previous Version Document: ${job.previousFileName}\n\nJob ID: ${job.id}\nThis is a simulated download.\nIn production, this would download from: ${job.previousFilePath}`;
                    const blob = new Blob([fileContent], { type: 'application/octet-stream' });
                    const url = window.URL.createObjectURL(blob);
                    const a = document.createElement('a');
                    a.href = url;
                    a.download = job.previousFileName;
                    document.body.appendChild(a);
                    a.click();
                    document.body.removeChild(a);
                    window.URL.revokeObjectURL(url);
            ...
            {selectedJob.hasPreviousVersion && (
              <div className="p-4 bg-blue-50 border-2 border-blue-200 rounded-lg mb-4">
                <p className="text-sm text-blue-800">
                  <strong>📋 Version Comparison Requested:</strong> The client has provided both current and previous document versions. Please include a "What Changed" section at the start of the course.
                </p>
              </div>
            )}
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 p-6">
        <div className="max-w-6xl mx-auto">
          ...
          {/* Submitted Quiz Questions - if manual or hybrid mode */}
          {selectedJob.manualQuestions && selectedJob.manualQuestions.length > 0 && (
            <div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
              <h3 className="text-xl font-bold text-gray-900 mb-4">📝 Submitted Quiz Questions ({selectedJob.manualQuestions.length})</h3>
              <div className="space-y-4">
                {selectedJob.manualQuestions.map((q: any, idx: number) => (
                  <div key={q.id || idx} className="p-4 bg-yellow-50 border-2 border-yellow-200 rounded-lg">
                    <p className="font-semibold text-gray-900 mb-2">{idx + 1}. {q.question}</p>
                    <div className="ml-4 space-y-1">
                      {q.options.map((opt: string, optIdx: number) => (
                        <p key={optIdx} className={`text-sm ${optIdx === q.correctAnswer ? 'text-green-700 font-semibold' : 'text-gray-600'}`}>
